...
void StaticMesh::Initialize(wstring _fp)
{
	...
	ThrowIfFailed(D3DXLoadMeshFromX(_fp.c_str(), D3DXMESH_MANAGED, m_device, &m_adjacency, &m_subset, NULL, &m_subsetCount, &m_originMesh));
	...
	//{
	//	m_vertexPositionData[i] = *(Vector3*)(((unsigned long*)vertex) + (m_stride * i + positionOffset));
	//}
	...
	m_indexData = new unsigned int[m_triCount * 3];
	//for (unsigned int i = 0; i < m_triCount * 3; ++i)
	//{
	//	m_indexData[i] = (unsigned int)*((USHORT*)(((unsigned short*)indices) + (sizeof(unsigned short) * i)));
	//}
