...
void StageSelectScene::Initialize()
{
	...
	auto cam = INSTANTIATE()->AddComponent<Camera>()->AddComponent<MoveBySpline>()->SetPosition(0,0,0);
	cam->GetComponent<MoveBySpline>()->AddSplinePoint({ 0,0,0 })->AddSplinePoint({ 0,5,3 })->AddSplinePoint({ 2,6,30 })->AddSplinePoint({ 7,12,-40 })->AddSplinePoint({ -35,-6,20 })->AddSplinePoint({ 50,15,30 })->AddSplinePoint({ 70,30,50 });
	{
