...
{
	if (!m_Instance)
	{//?ÖÎç∞?¥Ìä∏Î•??∞Í∏∞ ?ÑÌï¥???ºÎ? ComponentÎ•??ÅÏÜçÎ∞õÏ? Ïª¥Ìè¨?åÌä∏ Í∞ùÏ≤¥Î£®Îã§Í∞Ä ÎßåÎì¶.

		auto Instance = INSTANTIATE();
		Instance->AddComponent<EnemyManager>();
		m_Instance = Instance->GetComponent<EnemyManager>();

		Instance->SetDontDestroy(true);//???òÏñ¥Í∞Ä??ÏßÄ?∞Ï?ÎßàÎùº ?¥Í±∞??
	}
...
int EnemyManager::Get_EnemyCount() const
{
	...
	//ÏµúÏ†Å?îÎ? ?ÑÌï¥??Í∑∏ÎÉ• Î©§Î≤Ñ Î≥Ä?òÎ£®?§Í? ?òÍ∏∞??
